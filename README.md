  ...
  + ## 라이프사이클
    + 스프링의 빈 생명주기 콜백 지원 3가지
      + 인터페이스(InitializingBean, DisposableBean)
        + 특징 : 인터페이스에 구현된 메소드 명으로만 가능 [afterPropertiesSet(), destroy()]
      + 설정 정보에 초기화 메서드 ,종료 메서드 지정
        + 특징 : 
          + 메서드 이름을 자유롭게 줄 수 있다.
          + 스프링 빈이 스프링 코드에 의존하지 않는다.
          + 코드가 아니라 설정 정보를 사용하기 때문에 코드를 고칠 수 없는 외부 라이브러리에도 초기화, 종료 메서드를 저용할 수 있다.
      + <U>**@PostConstruct**</U>, <U>**@PreDestroy**</U> 애노테이션 지원
        + 특징 :
          + 최신 스프링에서 가장 권장하는 방법이다.
          + 패키지는 <U>**javax.annotation.PostConstruct**</U> 하위 이다. 스프링에 종속적인 기술이 아니라 <U>**JSR-550**</U>이라는 자바 표준이다.  
          + 따라서 스프링이 아닌 다른 컨테이너에도 작동 가능하다.
          + 컴포넌트 스캔과 잘 어울린다.
          + 유일한 단점은 외부 라이브러리에는 적용하지 못한다는 것이다. 외부 라이브러리를 초기화, 종료 해야하면 @Bean의 기능을 사용하자.
      + 정리 : 
        + <U>**@PostConstruct**</U>, <U>**@PreDestroy**</U> 애노테이션을 사용하자!
        + 코드를 고칠 수 없는 외부라이브러리를 초기화, 종료 해야하면 @Bean의 <U>**initMethod**</U>, <U>**destroyMethod**</U>를 사용하자 
    + ### 스프링 빈은 간단하게 다음과 같은 <U>**라이프사이클**</U>을 가진다.
      + <U>**객체 생성**</U> -> <U>**의존관계 주입**</U> 
        + <U>**(생성자 주입은 예외다. 왜냐하면 객체를 만들 때 이미 스프링 빈이 같이 들어와야 하기 때문에 파라미터의 예외이다.)**</U>
      + <U>**스프링 빈은 객체를 생성하고, 의존관계 주입이 다 끝난 다음에야 필요한 데이터를 사용할 수 있는 준비가 완료된다.**</U>
      + 따라서 초기화 작업은 의존관계 주입이 모두 완료되고 난 다음에 호출해야 한다.
      + 스프링은 의존관계 주입이 완료되면 스프링 빈에게 <U>**초기화 콜백 메서드**</U>를 통해서 초기화 시점을 알려주는 다양한 기능을 제공한다.  
        또한 스프링은 스프링 컨테이너가 종료되기 직전에 <U>**소멸 콜백**</U>을 준다. 따라서 안전하게 종료 작업을 진행할 수 있다.
    + 스프링 빈의 이벤트 사이클
    ...
    ![img.png](images/Spring-Bean-lifeCycle.png)
  + ## 빈 스코프
    + 설명 : 빈이 존재할 수 있는 범위를 뜻한다.
    + 스코프 종류 :
      + <U>**싱글톤**</U> : 기본 스코프, 스프링 컨테이너의 시작과 종료까지 유지되는 가장 넓은 범위의 스코프이다.
        + 빈 조회시 항상 같은 인스턴스의 스프링 빈을 반환한다.
      + <U>**프로토타입**</U> : 스프링 컨테이너는 프로토아입 빈의 생성과 의존관계 주입까지만 관여하고 더는 관리하지 않는 매우 짧은 범위 스코프이다.
        + 빈 조회시 항상 새로운 인스턴스의 스프링 빈을 반환한다.
        + 스프링 컨테이너는 프로토타입 빈을 <U>**생성하고**</U>, <U>**의존관계 주입**</U>, <U>**초기화**</U> 까지만 처리하고 클라이언트에 반환한다.
          하여 클라이언트에서 종료를 원할 시 <U>**@PreDestroy**</U>를 호출해 줘야 한다.
      + <U>**웹 관련 스코프**</U> :
        + <U>**request**</U> : 웹 요청이 들어오고 나갈 때 까지 유지되는 스코프이다.
        + <U>**session**</U> : 웹 세션이 생성되고 종료될 때 까지 유지되는 스코프이다.
        + <U>**application**</U> : 웹의 서블릿 컨텍스와 같은 범위로 유지되는 스코프이다.
  + <span style="color:red">********* <U>**싱글톤 빈과 프로토타입 빈을 동시에 쓸 시 주의점**</U> ********* </span>
    + 처음 스프링이 실행될 때 싱글톤 타입과 프로토타입이 같이 IOC에 생성이 되면
        ...
        + request 사용시 클래스명 그대로 DI를 실행하면 request가 들어오지도 안았는데 DI를 요청해서 에러가 난다.
          + 해결 방법
            ...
            + 2.  <U>**proxy 모드**</U>를 사용하자
              ...
              + 실제 request 요청이 왔응 때 scope로 등록된 진짜 Bean을 조회한다.
...
+ # 애노테이션
  ...
  + 그러나 스프링이 제공하는 기능을 뚜렷한 목적 없이 무분별하게 재정의 하는 것은 유지보수에 더 혼란만 가중할 수 있다.
